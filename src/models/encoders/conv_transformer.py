...
from src.utils.utils_models import time_embedding
from src.utils.utils_models import PositionalEncoder
from src.utils.base_layers import LayerNormGeneral
...
class DownSampling(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm,  post_norm=partial(LayerNormGeneral, bias=False, eps=1e-6)):
        super().__init__()
        self.dim = dim
...
class ConvFormerSits(nn.Module):
    ...
    def __init__(
        ...
        if positional_encoding:
            self.positional_encoder = PositionalEncoder(
                self.d_model // num_heads[-1], T=T, repeat=num_heads[0]
            )
        else:
    ...
    def forward(self, x, batch_positions):
        """Forward function."""
        # print('self.merge_after_stage:',  self.merge_after_stage) # torch.Size([4, 6, 10, 10, 10])

        self.nbts = x.size(1)  # To be removed
        # print('size dates:', dates.shape) torch.Size([4, 6, 10, 10, 10])
        ...
        x = x + time_embed
        # print("Time + spatial embeddings:", time_embed.shape)
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                if (
                    ...
                    if self.config["models"]["t_convformer"]["sits_temp_merging"] == "mean":
        ...
        # print("outs outputs", outs[0].shape)
        # print("outs outputs", outs[1].shape)
        # print("outs outputs", outs[2].shape)
        # print("outs outputs", outs[3].shape)
        ...
        # print("merged_time outputs", merged_time[0].shape)
        # print("merged_time outputs", merged_time[1].shape)
        # print("merged_time outputs", merged_time[2].shape)
